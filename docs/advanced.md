...
### ⚠️ 落とし穴：作業スペースは“1つ”を共有している

タブを分けても、**手元の作業フォルダ（ワークツリー）が1つのまま** だと、事故が起きます。

ワークツリーを使わずに、こうするとどうなるでしょう——

- **Aのタブ**で `main` を修正中
- **Bのタブ**で `feat-1` ブランチを切って作業開始
- → 作業フォルダ全体が `main` → `feat-1` に **切り替わる**
- → **Aで編集していた内容まで `feat-1` に乗ってしまう** 😱

```
1つの作業フォルダを A・B のタブが共有
 ├ Aタブ: main を編集中
 └ Bタブ: feat-1 に切り替え → フォルダ全体が feat-1 に
 結果：Aの作業も feat-1 に乗ってしまう ⚠️
```

（前の「ブランチはローカルでどうなっている？」で見た **“作業フォルダは1つ”** が、ここで効いてきます。）

**だから：タブごとに別ブランチで作業するなら、ワークツリーで作業フォルダも分けます。**

#### 分ける／分けないの目安

```mermaid
flowchart TD
    Q{タブごとに別ブランチで作業する？} -->|はい| W[ワークツリーで分ける<br/>各タブ＝別フォルダ＝別ブランチ]
    Q -->|いいえ：同じブランチで<br/>関係ない箇所だけ触る| O[1つの作業スペースでOK]
```

- 別ブランチで並行、または **修正箇所が競合しそう** → **ワークツリーで分ける**
- 同じブランチで、**まったく関係ない箇所** だけ触る → 1つの作業スペースでOK

!!! note "Bのタブは、Aのタブの作業を知りません"
    別々のClaude会話は、互いの作業内容を参照できません。
    競合しそうなときは、**ブランチを切ってワークツリーで分ける** のが安全です。

!!! tip "補足：ワークツリーとブランチの関係"
    - ワークツリーを作る ≒ **別ブランチも用意** する（ただし **既存のブランチでワークツリーを作る** こともできます）
    - **同じブランチに複数のワークツリーは作れません**（例：`main` にワークツリー A と B、は不可）
