...
/* 見本はグリッドで整列。列数は幅に合わせて自動（2〜3列）、列はぴったり揃う。 */
.dv-grid{display:grid;grid-template-columns:repeat(auto-fill,240px);gap:22px;margin:1.4em 0;justify-content:center;}
...
**結論：言葉だけより、「こういうのが好き（参考）」と「これは避けたい（NG）」をセットで伝えると、ねらいに一気に近づきます。**

**理由：** デザインの好みは人それぞれ。AIは「ふつう」が分かりません。
**進みたい方向（◎）**と**避けたい方向（×）**の両方を示すと、AIは“当たり”の範囲をしぼり込めます。

### ① 「好き」を見せる（参考・ムードボード）

- **参考ページのURLを渡す**：`このサイトみたいな雰囲気で：https://example.com`
- **複数集めて渡す**（ムードボード）：`この3つの“ここが好き”を混ぜたい。Aの配色、Bの余白、Cのボタンの形。`
- **スクショを渡す**：気に入った画面を貼って `この感じで、中身は別物を作って`

!!! tip "「全部マネ」ではなく「いいとこ取り」で頼む"
    「**Aの色 ＋ Bのレイアウト**」のように、**要素ごとに**「どこが好きか」を言うと、ただのコピーになりません。

### ② 「避けたい（NG）」を伝える

「やってほしくないこと」を先に言うと、手戻りが減ります。

| 避けたい方向 | こう伝える |
|---|---|
| 派手・うるさい | `派手すぎないように。色は3色までで` |
| 子どもっぽい | `かわいくしすぎず、落ち着いた大人向けで` |
| 文字が小さい・読みにくい | `文字は大きめ、読みやすさ優先で` |
| ごちゃごちゃ | `要素を詰め込みすぎず、余白を多めに` |
| 古い印象 | `今っぽく。古くさい感じは避けて` |

```text
モダンで落ち着いたカフェのトップ画面を作って。
◎ 好き：余白広め、ナチュラルな色、明朝の見出し
× 避けたい：派手な色、文字が小さい、ごちゃごちゃ
参考：https://example.com（この余白感が好き）
```

!!! warning "参考画像・ロゴをそのまま使わない"
    気に入ったサイトの**画像やロゴをそのまま使う**のは、著作権・ライセンス上の問題になることがあります。
    「**雰囲気を参考に、中身は自分のものを作る**」のが安全です。リンクやスクショを渡すときも、**実在の顧客名・案件名・社外秘**が写り込んでいないか確認しましょう。
...
- [x] **「好き（参考）」と「避けたい（NG）」**をセットで伝えられる
- [x] プロンプトに**実在の顧客名・機密**は書かない

!!! success "次のステップ"
    作る中では **エラー** にも出会います。次は、エラーとの上手な付き合い方を覚えましょう。
